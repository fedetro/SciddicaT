...
/*
  con l'utilizzo di CUDA Unified Memory, il metodo addLayer2D non è più necessario nel suo formato 
  originale, poiché la gestione della memoria è semplificata grazie alla memoria unificata. Invece di 
  allocare manualmente la memoria per ciascun sottostato (come Sz, Sh, e Sf), è possibile allocare e 
  deallocare la memoria direttamente sulla GPU utilizzando le funzioni di allocazione e deallocazione di CUDA.
*/
...
int main(int argc, char **argv)
{
  ...
  // Allocazione e inizializzazione della memoria GPU unificata
  double *d_Sz, *d_Sh, *d_Sf;
  cudaMallocManaged(&d_Sz, sizeof(double) * r * c);
  cudaMallocManaged(&d_Sh, sizeof(double) * r * c);
  cudaMallocManaged(&d_Sf, sizeof(double) * (SIZE_OF_X - 1) * r * c);
  ...
  // Deallocazione della memoria GPU unificata
  cudaFree(d_Sz);
  cudaFree(d_Sh);
  cudaFree(d_Sf);
